...
export async function POST(
  ...
  try {
    ...
    const { data: committeeMember, error: cmError } = await getSupabaseAdmin()
    ...
    const { error: approvalError } = await getSupabaseAdmin()
    ...
    try {
      const { data: expense } = await getSupabaseAdmin()
        .from("expenses")
        .select("title")
        .eq("id", id)
        .single()

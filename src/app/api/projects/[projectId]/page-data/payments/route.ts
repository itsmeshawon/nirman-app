...
export async function GET(
  ...
  const [{ data: scheduleItems }, { data: payments }, { data: milestones }, { data: paymentProofs }] = await Promise.all([
    ...
    supabase.from("payments").select("*, shareholder:shareholders(id, unit_flat, profiles(name, phone)), recorded_by:profiles!recorded_by_id(name), schedule_item:schedule_items(milestone:milestones(name)), proof:payment_proofs(attachment_url, attachment_name)").in("shareholder_id", safeSh).order("created_at", { ascending: false }),

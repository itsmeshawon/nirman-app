...
export async function DELETE(
  ...
  try {
    ...
    await supabaseAdmin.from("notifications").delete().eq("project_id", projectId)
    ...
    if (postIds.length > 0) {
      await supabaseAdmin.from("reactions").delete().in("post_id", postIds)
      await supabaseAdmin.from("post_views").delete().in("post_id", postIds)
    }

    // ─── Step 3: Activity posts ───────────────────────────────────────────────
    await supabaseAdmin.from("activity_posts").delete().eq("project_id", projectId)
    ...
    if (expenseIds.length > 0) {
      await supabaseAdmin.from("expense_attachments").delete().in("expense_id", expenseIds)
      await supabaseAdmin.from("expense_approvals").delete().in("expense_id", expenseIds)
    }

    // ─── Step 5: Expenses ─────────────────────────────────────────────────────
    await supabaseAdmin.from("expenses").delete().eq("project_id", projectId)

    // ─── Step 6: Payments ────────────────────────────────────────────────────
    await supabaseAdmin.from("payments").delete().eq("project_id", projectId)
    ...
    if (scheduleIds.length > 0) {
      ...
      if (scheduleItemIds.length > 0) {
        await supabaseAdmin.from("penalties").delete().in("schedule_item_id", scheduleItemIds)
      }
      await supabaseAdmin.from("schedule_items").delete().in("schedule_id", scheduleIds)
    }

    // ─── Step 8: Payment schedules ───────────────────────────────────────────
    await supabaseAdmin.from("payment_schedules").delete().eq("project_id", projectId)

    // ─── Step 9: Milestones ──────────────────────────────────────────────────
    await supabaseAdmin.from("milestones").delete().eq("project_id", projectId)

    // ─── Step 10: Penalty configs ─────────────────────────────────────────────
    await supabaseAdmin.from("penalty_configs").delete().eq("project_id", projectId)

    // ─── Step 11: Committee members ───────────────────────────────────────────
    await supabaseAdmin.from("committee_members").delete().eq("project_id", projectId)

    // ─── Step 12: Shareholders ────────────────────────────────────────────────
    await supabaseAdmin.from("shareholders").delete().eq("project_id", projectId)

    // ─── Step 13: Project admins ──────────────────────────────────────────────
    await supabaseAdmin.from("project_admins").delete().eq("project_id", projectId)

    // ─── Step 14: Expense categories ──────────────────────────────────────────
    await supabaseAdmin.from("expense_categories").delete().eq("project_id", projectId)

    // ─── Step 15: Approval & notification configs ─────────────────────────────
    await supabaseAdmin.from("notification_configs").delete().eq("project_id", projectId)
    await supabaseAdmin.from("approval_configs").delete().eq("project_id", projectId)

    // ─── Step 16: Project documents ───────────────────────────────────────────
    await supabaseAdmin.from("project_documents").delete().eq("project_id", projectId)
